...
class mmc5983(self, i2c_bus, addr):

    def __init__(self, i2c_bus, addr):
        self.i2c_device = I2CDevice(i2c_bus, addr, probe=False)
        self.i2c_addr = addr
        product_id = self.id_verify(self)
        if(product_id != PRODUCT_ID_1_VALUE):
            print("[ERROR][IAM23080 @ ADDR: {addr}][PRODUCT_ID_1 VALUE {product_id}]")
